    ...
    if (event.entering) {
      switch(type) {
        ...
        case 'code_block':
          ...
          htmlBlock.literal = `<code-listing><source-code lang="${node.info || 'JavaScript'}"><pre class="language-${highlightLanguage}"><code>${highlightCode(node.literal, highlightLanguage)}</code></pre></source-code></code-listing>`;
