...
exports.ArticleParser = class {
  ...
  parse() {
    ...
    let result = transformMarkdown(transformed);
...
function transformMarkdown(content) {
        ...
        case 'heading':
          if(node.level === 2) {
            if (sectionHeader !== null) {
              node.insertBefore(wrapInSection(sectionHeader, sectionId, sectionVersion, node, headers));
            }
  ...
  if (sectionHeader !== null) {
    parsed.appendChild(wrapInSection(sectionHeader, sectionId, sectionVersion, node, headers));
  }
...
function wrapInSection(sectionHeader, sectionId, sectionVersion, node, headers) {
  let htmlBlock = new commonmark.Node('html_block');
