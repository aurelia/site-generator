...
class ModuleWriter {
  ...
  write(render, outDir) {
    ...
    if (this.api.exampleSrc) {
      fs.copyFolderRecursiveSync(this.api.exampleSrc, path.join(outDir, this.api.exampleDest), true);
    }
...
function writeMember(root, apiRoot, model, render, template) {
  ...
  writePromises.push(fs.writeFile(htmlDest, render(json.name + " " + json.kindString, html)));
  writePromises.push(fs.writeFile(htmlFragmentDest, html));
