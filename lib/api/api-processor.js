...
class ModuleWriter {
  ...
  write(render, outDir) {
    ...
    writePromises.push(fs.writeFile(htmlDest, render(this.model.name + " Module", html)));
    writePromises.push(fs.writeFile(htmlFragmentDest, html));
    writePromises.push(fs.writeFile(jsonDest, JSON.stringify(this.json)));

    this.model.classes.forEach(x => writePromises.push(writeMember(root, x, render, classOrInterfaceTemplate)));
    this.model.interfaces.forEach(x => writePromises.push(writeMember(root, x, render, classOrInterfaceTemplate)));
    this.model.constants.forEach(x => writePromises.push(writeMember(root, x, render, constantOrPropertyTemplate)));
    this.model.functions.forEach(x => writePromises.push(writeMember(root, x, render, functionOrMethodTemplate)));

    return Promise.all(writePromises);
  }
}

function writeMember(root, model, render, template) {
  root = path.join(root, model.kindString.toLowerCase(), model.name);
  let json = createMemberJSON(root, model);
  ...
  writePromises.push(fs.writeFile(json.links.html, render(model.name + " " + model.kindString, html)));
  writePromises.push(fs.writeFile(json.links.fragment, html));
  writePromises.push(fs.writeFile(json.links.self, JSON.stringify(json)));

  if (model.properties) {
    model.properties.forEach(x => writePromises.push(writeMember(root, x, render, constantOrPropertyTemplate)));
  }

  if (model.methods) {
    model.methods.forEach(x => writePromises.push(writeMember(root, x, render, functionOrMethodTemplate)));
  }

  return Promise.all(writePromises);
}

function createMemberJSON(root, model) {
  return Object.assign({
    links: {
      static: root,
      html: path.join(root, 'index.html'),
      fragment: path.join(root, 'index-fragment.html'),
      self: path.join(root, 'index.json')
    }
  }, model);
}
