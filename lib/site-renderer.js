...
const {DefaultProcessor} = require('./processors/default/default-processor');
...
exports.generateSite = function(project) {
  ...
  if (project.learn) {
    processors.push(new ArticleProcessor(project.learn));
  }

  if (project.support) {
    processors.push(new ArticleProcessor(project.support));
  }

  if (project.pages) {
    project.pages.forEach((page) => {
      processors.push(new DefaultProcessor(page));
    });
  }
...
function runProcessors(project, render, processors) {
  ...
  return Promise.all(processors.map(p => p.process(context)))
    ...
    .then(() => {
      let scriptPath = require.resolve(`./app/scripts/${appSrc}`);
      return fs.copy(scriptPath, path.join(project.outDir, `scripts/${appSrc}`));
    })
